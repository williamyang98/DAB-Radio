...
{
private:
    using Base = ViterbiDecoder_Core<error_t, soft_t, decision_bits_t, absolute_error_t>;    
public:
    template <typename ... U>
...
private:
    ...
    void bfly(const soft_t* symbols, decision_bits_t* decision, error_t* old_metric, error_t* new_metric) {
        for (size_t curr_state = 0u; curr_state < this->branch_table.stride; curr_state++) {
            // Error associated with state given symbols
            error_t total_error = 0u;
            for (size_t i = 0; i < this->R; i++) {
                const soft_t sym = symbols[i];
                const soft_t expected_sym = this->branch_table[i][curr_state];
                const soft_t error = expected_sym - sym;
                const error_t abs_error = error_t(abs(error));
                total_error += abs_error;
            }
            assert(total_error <= this->config.soft_decision_max_error);

            // Butterfly algorithm
            const error_t m_total_error = this->config.soft_decision_max_error - total_error;
            ...
            const error_t m1 = old_metric[curr_state + this->METRIC_LENGTH/2] + m_total_error;
            const error_t m2 = old_metric[curr_state                        ] + m_total_error;
            const error_t m3 = old_metric[curr_state + this->METRIC_LENGTH/2] +   total_error;
            ...
            const size_t curr_pack_index = curr_bit_index / this->DECISIONTYPE_BITSIZE;
            const size_t curr_pack_bit   = curr_bit_index % this->DECISIONTYPE_BITSIZE;
    ...
    void renormalise(error_t* metric) {
        ...
        for (size_t curr_state = 1u; curr_state < this->METRIC_LENGTH; curr_state++) {
        ...
        for (size_t curr_state = 0u; curr_state < this->METRIC_LENGTH; curr_state++) {
            error_t& x = metric[curr_state];
            x -= min;
        }
