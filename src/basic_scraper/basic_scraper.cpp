...
void BasicAudioScraper::OnAudioData(BasicAudioParams params, const uint8_t* data, const int N) {
    ...
    const int nb_written = fwrite(data, sizeof(uint8_t), N, fp_wav);
    if (nb_written != N) {
...
void BasicSlideshowScraper::OnSlideshow(mot_transport_id_t id, Basic_Slideshow* slideshow) {
    ...
    const int nb_written = fwrite(slideshow->data, sizeof(uint8_t), slideshow->nb_data_bytes, fp);
...
void BasicMOTScraper::OnMOTEntity(MOT_Entity* mot) {
    ...
    const int nb_written = fwrite(mot->body_buf, sizeof(uint8_t), mot->nb_body_bytes, fp);
