...
std::shared_ptr<Device> RenderDeviceList(DeviceList& device_list, Device* device) {
    ...
    int selected_index = -1;
    {
        ...
        auto label = fmt::format("Devices ({})###Devices", descriptors.size());
        if (descriptors.size() == 0) ImGui::BeginDisabled();
        if (ImGui::BeginCombo(label.c_str(), preview_label.c_str())) {
    ...
    if (selected_index != -1) {
        return device_list.get_device(size_t(selected_index)); 
    }
    return nullptr;
}
